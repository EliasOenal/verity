...
export class NetworkManager extends EventEmitter implements NetworkManagerIf {
    ...
    offerCubesToConnectedPeers(cubeInfos: CubeInfo[]): void {
        ...
        if (fullNodePeers.length === 0) {
            logger.trace('NetworkManager.offerCubesToConnectedPeers(): No connected full nodes to offer cubes to');
            return;
        }
        ...
        for (const peer of fullNodePeers) {
            try {
                ...
                logger.warn(`NetworkManager.offerCubesToConnectedPeers(): Error sending cube offer to peer ${peer.toString()}: ${error}`);
            }
        }
    }
