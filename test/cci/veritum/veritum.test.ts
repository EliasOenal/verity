...
describe('Veritum', () => {
  ...
  describe('construction', () => {
    describe('FromChunks()', () => {
      it.todo("note that we don't have any manual FromChunks() tests, but we do have round-trip compilation tests");
    });
    ...
    describe('round-trip tests', () => {
      for (const chunkNo of [1, 2, 3]) {
        for (const cubeType of enumNums(CubeType)) {
          ...
          for (const roundTrips of [1/*,2 TODO still failing*/]) describe(`performing ${roundTrips === 1? 'a single round trip': roundTrips + ' round trips'}`, () => {
                ...
                describe(`${chunkNo}-chunk ${CubeType[cubeType]} Veritum`, () => {
                  ...
                  beforeAll(async () => {
                    ...
                    if (roundTrips > 1) {
                      ...
                      const previousChunks = Array.from(reconstructed.chunks);
                      const previousBinaryData: Buffer[] = [];
                      for (const chunk of previousChunks) {
                        ...
                        previousBinaryData.push(Buffer.from(bin));
                      }
                      ...
                      expect(recompiledChunks.length).toEqual(previousChunks.length);
                      for (let i=0; i<recompiledChunks.length; i++) {
                        ...
                        const previousChunk: cciCube = previousChunks[i];
                        ...
                        // (should be equal though, will check for equality in the actual tests below)
                  ...
                  if (roundTrips > 1) {
                    it.todo('first and second compilation yield identical chunk keys');
                    it.todo('first and second compilation yield identical binary chunks');
                  }
                });  // feeding an unencrypted Veritum
              });  // describe combination of options
            }  // for encrypt & supplyKey of [true, false]
          });  // for/describe number of round trips
        }  // for cubeType
      }  // for chunkNo
    });  // round-trip tests
  });  // compile()
