 ...
 * It initializes a light node with testCoreOptions built-in for optimal test performance.
 */
...
async function initializeLightNodeTest(): Promise<void> {
  ...
  logger.info('Initializing light node test application');
  ...
  (window as any).verity = {
    ...
    testUtils: {
      createUniqueTestCube: async (content?: string) => {
        const uniqueContent = content || `LIGHT-${Date.now()}-${Math.random()}`;
      ...
      createMultipleTestCubes: async (count: number) => {
        const results = [];
        for (let i = 0; i < count; i++) {
          const result = await (window as any).verity.testUtils.createUniqueTestCube(`MULTI-${i}`);
          results.push(result);
  ...
  if (statusEl) {
    statusEl.textContent = 'Light node ready!';
  }
  ...
  logger.info('Light node test application ready');
}
...
if (isBrowser) {
  document.addEventListener('DOMContentLoaded', async () => {
    try {
      const statusEl = document.getElementById('status');
      if (statusEl) {
        statusEl.textContent = 'Initializing light node...';
      }
      
      await initializeLightNodeTest();
    } catch (error) {
      logger.error('Failed to initialize light node test:', error);
